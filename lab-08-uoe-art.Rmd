...
uoe_art <- read.csv("data/uoe_art.csv")
glimpse(uoe_art)
```
...
uoe_art_clean <- uoe_art %>%
  ...
  mutate(
    ...
    # grab a 4-digit year anywhere in the date string
...
ANSWER:  When I run the `separate()` and `mutate()` code, R prints some warnings about some rows not being split or some values being `NA`. These warnings occur because not every title contains parentheses or meaningful date information, so in those cases the `date` column ends up as `NA` or doesn’t contain a four-digit year for `str_extract()` to pick up. That’s acceptable, since we only want to capture a year when it is clearly available and are okay leaving `year` as missing when it’s not.
...
ANSWER: According to the summary, there are 3320 pieces with missing artist information and  900 pieces with missing year information.
...
uoe_art_clean %>%
  ...
  labs(
    title = "Distribution of artwork years in the Edinburgh College of Art collection",
    x = "Year",
    y = "Count"
  ) +
  theme_minimal()
```
  ANSWER: The histogram shows that most works in the collection are from the 19th and 20th centuries, with a heavy concentration in the mid-20th century. There is one clear outlier year that sits far away from the main cluster, which suggests that either the date was recorded differently or our extraction code mis-parsed the year for that piece.
